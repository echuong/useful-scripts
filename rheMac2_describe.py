...
def main(argv=None):
	...
	required.add_option('-g', '--geneID', help="geneID file", dest='geneIDfile', action='store', default="/data/genomes/Homo_sapiens/hg19/hg19.xref.txt", metavar='[/data/genomes/Homo_sapiens/hg19/hg19.xref.txt]')
	required.add_option('-c', '--column', help="infile ID column, 1-based [%default]", dest='col', default=2,  metavar='[# things]', type='int', action='store')
	required.add_option('-x', '--xrefcolumn', help="xref column, 1-based (1 is kgid, 2 is genbank mRNA) [%default]", dest='xcol', default=1,  metavar='[# things]', type='int', action='store')
